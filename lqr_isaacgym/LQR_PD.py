...
class isaacgym_test():
    def __init__(self):
        ...
        self.st_noise_cart_vel = 0.1
        self.st_noise_pole_pos = 0.0
        self.st_noise_pole_vel = 0.3

        self._get_noise_scale_vec()

        #LQR
        self.ref = torch.tensor([0, 0, 0, 0], dtype=torch.float, device=self.device) # Desired state
    ...
    def compute_lqr_gain(self): 
        ...
        m_p = 0.2      # Pendulum (rod) mass [kg] 
        m_w = 0.0       # Point mass at end [kg]
        M   = 0.694     # Cart mass [kg]
        l   = 0.65      # Total pendulum length [m] (pivot to tip)
        g   = 9.8065    # Gravity [m/s^2]

        # Derived properties
        m = m_p + m_w                          # Total pendulum mass
        L = (0.5 * m_p * l + m_w * l) / m      # CoM from pivot
        I = (1/3 * m_p * l**2 + m_w * l**2)    # Moment of inertia about pivot

        # Denominator (mass matrix determinant)
        Delta = (I + m * L**2) * (M + m) - (m * L)**2

        # State-space matrices (linearized around upright θ = π)
        A = np.array([
            [0, 1, 0, 0],
        ...
        #  the discretization using a simple forward Euler approximation
        Add = np.array([
            [1, dt, 0, 0],
            [0, 1, -(L**2 * m**2 * g * dt) / Delta, 0],
            [0, 0, 1, dt],
            [0, 0,  (L * m * g * (M + m)*dt) / Delta, 0]
        ])
        ...
        # Discretize the continuous system using ZOH
        sysd = cont2discrete((A, B, C, D), dt, method='zoh')
        Ad, Bd, Cd, Dd, _ = sysd

        Q = np.diag([0.8, 0.05,  1.0, 0.05])
        R = np.array([[0.001]])

        self.Kd, _, eigValsd = self.dlqr(Ad, Bd, Q, R) # ZOH
        self.Kdd, _, eigValsdd = self.dlqr(Add, Bdd, Q, R) # Euler approximation
        self.KK, _, eigVals = self.lqr(A, B, Q, R) # continuous


        self.K = torch.tensor(self.Kd, dtype=torch.float, device=self.device)

        # print('k conti:', self.KK)
        print('k EU disc:', self.K)
        # print('k ZOH disc:', self.Kd)
    ...
    def run(self):
        ...
        try:
            for step in range (int(MAX_ITR)):
                ...
                # if step%500==0:
                #     self._push_force(force=40)
                #     print('********push force!!!!step%100**********')
                    # elif step%70==0:
                    #     self._push_force(force=0)
                    #     print('********push force!!!!step%70**********')
                
                self.x_des = -self.K @ (self.states - self.ref).T 
                self.u = KP * (self.x_des - self.states[0, 0]) - KD * self.states[0, 1]
                self.u = self.u.clone().detach().to(dtype=torch.float, device=self.device) 
                self.u = torch.clamp(self.u, min=-200.0, max=200.0)
                ...
                self.xdes_log.append(self.x_des[0].cpu().numpy().copy())
                self.control_log.append(self.u[0].cpu().numpy().copy())
                self.time_log.append(step * 0.002)
                iter_end = time.time()
                iter_duration = iter_end - iter_start
                first_mid = iter_mid - iter_start
                second_mid = iter_end - iter_mid
                # print("iter whle", iter_duration, "iter first mid",  first_mid,"iter second mid", second_mid)
    ...
    def _create_envs(self):
        ...
        dof_states['pos'][1] = -np.pi/12 # Assuming index 1 is the cart-to-pole joint
        self.gym.set_actor_dof_states(env_handle, actor_handle, dof_states, gymapi.STATE_ALL)

        # Set DOF properties
        dof_props = self.gym.get_asset_dof_properties(robot_asset)
        dof_props['stiffness'][:] = 0.05
        dof_props['damping'][0] = 0.4   # slider to cart
        dof_props['damping'][1] = 0.003 # cart to pole
        dof_props['friction'][0] = 0.3  # slider to cart 
        dof_props['friction'][1] = 0.000 # cart to pole
    ...
    def save_log(self):
        ...
        log_df = pd.DataFrame({
            ...
            'x_des': [x[0] for x in self.xdes_log],
        })
    ...
    def plot_log(self):
        ...
        for column in ['x', 'x_dot', 'theta', 'theta_dot', 'torque', 'x_des']:
