...
class isaacgym_test():
    def __init__(self):
        ...
        # SG_filter
        self.window_length = 31 # Odd, minimal delay (~6 ms)
    ...
    def compute_states(self): 
        ...
        # SG_filter
        raw_angle = self.dof_pos[0, 1].item() + np.pi
        raw_angle_tensor = torch.tensor(raw_angle, device=self.device, dtype=torch.float32)

        quantized_angle = torch.round(raw_angle_tensor / ENCODER_RES_RAD) * ENCODER_RES_RAD

        self.angle_buffer.append(raw_angle_tensor)
        print('**angle buffer:',len(self.angle_buffer))
        print('**windows length:', self.window_length)
        # if len(self.angle_buffer) == self.window_length:
        #     # velocity = np.dot(self.sg_filter()[::-1], list(self.angle_buffer))
        #     # velocity = np.dot(
        #         # self.sg_filter()[::-1],
        #         # [angle.cpu().item() if isinstance(angle, torch.Tensor) else angle for angle in self.angle_buffer])
        #     print('we are in control loop!')

        #     velocity = np.dot(coeffs[::-1], list(self.angle_buffer))
        #     self.velocity_estimate = torch.tensor(velocity, device=self.device, dtype=torch.float32)
        #     self.velocity_estimates.append(velocity)
        # else:
        #     self.velocity_estimates.append(0.0)  # Initial dummy value
        ...
        self.states = torch.cat((    
                                self.dof_pos[:, 0].unsqueeze(1), # cart
                                self.dof_vel[:, 0].unsqueeze(1),
                                quantized_angle.view(1, 1), # pole
                                self.velocity_estimates.view(1,1)
                                ),dim=-1)
        # torch.full((self.dof_pos.size(0), 1), self.velocity_estimates_tensor[-1], device=self.device)
    ...
    def sg_filter(self):
        ...
        coeffs = savgol_coeffs(self.window_length, polyorder=3 , deriv=1, delta=dt, use='conv')
    ...
    def _init_buffers(self):
        ...
        self.angle = torch.tensor(0.0, device=self.device)
        self.angle_buffer = deque(maxlen = self.window_length)
